...
export function buildRuntimeServicesSystemSuffix(): string | undefined {
  ...
  if (info.mode) {
    lines.push(
      `You are running inside a BoostersDev dev stack started in '${info.mode}' mode.`,
    );
  } else {
    lines.push("You are running inside a BoostersDev dev stack.");
  }
  lines.push(
  ...
  if (automation?.url_from_agent) {
    lines.push(
      ...
      `    ${automation.description ?? "BoostersDev Automations service."}`,
    );
...
export function buildDevServerSystemSuffix(): string {
  return [
    ...
    "  framework doesn't print it, echo it yourself. Also end your chat reply",
    "  with a line exactly like `Preview URL: http://localhost:PORT/` — Agent",
    "  Canvas reads that to load the preview automatically.",
