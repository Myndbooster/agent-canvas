...
export class AgentServerUnsupportedVersionError extends AgentServerUnavailableError {
  ...
  constructor(actualVersion: string) {
    const message = `BoostersDev requires agent-server ${MINIMUM_COMPATIBLE_AGENT_SERVER_VERSION} or newer; this backend is running ${actualVersion}. Please upgrade the agent-server backend.`;
    super(message);
...
export class AgentServerUnknownVersionError extends AgentServerUnavailableError {
  ...
  constructor(actualVersion: string | null) {
    ...
    const message =
      ...
      `BoostersDev requires agent-server ${MINIMUM_COMPATIBLE_AGENT_SERVER_VERSION} ` +
